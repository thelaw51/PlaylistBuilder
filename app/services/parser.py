...
def parse_library(file_path: str) -> int:
    ...
    for pl_data in playlists_data:
        ...
        playlist = Playlist.query.filter_by(itunes_id=itunes_id, library_id=_LIBRARY_ID).first()
        if not playlist:
...
def parse_playlist_txt(file_path: str, playlist_name: str) -> int:
    ...
    playlist = Playlist.query.filter_by(itunes_id=pl_itunes_id, library_id=_LIBRARY_ID).first()
    if not playlist:
        playlist = Playlist(library_id=_LIBRARY_ID, name=playlist_name, itunes_id=pl_itunes_id)
        db.session.add(playlist)
        db.session.flush()
