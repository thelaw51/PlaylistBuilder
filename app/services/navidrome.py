...
def wait_for_scan(timeout_seconds: int = 600) -> None:
    """Block until Navidrome reports scanning=false or timeout is reached."""
    with httpx.Client(timeout=10) as client:
...
def find_track_id(title: str, artist: str) -> str | None:
    """Return the Navidrome song ID for the best match, or None."""
    query = f"{artist} {title}".strip()
    with httpx.Client(timeout=10) as client:
        data = _get(client, "search3", query=query, songCount=5, albumCount=0, artistCount=0)
...
def create_or_update_playlist(name: str, song_ids: list[str]) -> str | None:
    ...
    with httpx.Client(timeout=30) as client:
        ...
        # Build repeated (songId, value) pairs — httpx accepts list of tuples
